            ...
            if isinstance(step[0], AgentAction) or include_final_answer:
                ...
                if include_agent_log:
                    ...
                    if isinstance(step[0], AgentAction):
                        ...
                        # include final answer or quit response
                        assert isinstance(step[0], AgentFinish)
                        if "output" in step[0].return_values and step[0].return_values["output"].startswith("QUIT:"):
                            trajec += f"QUIT: {step[0].return_values['output'][5:].strip()}\n"
                        else:
            ...
            if i != len(traject_dict["intermediate_steps"]) - 1 or include_final_answer:
                ...
                if include_agent_log:
                    ...
                    if i != len(traject_dict["intermediate_steps"]) - 1:
                        ...
                        # include final answer or quit response, robust to empty or non-string
                        if isinstance(step[0], list) and len(step[0]) > 0 and isinstance(step[0][-1], str):
                            if step[0][-1].startswith("QUIT:"):
                                trajec += f"QUIT: {step[0][-1][5:].strip()}\n"
                            else:
