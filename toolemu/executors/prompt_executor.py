...
class BasePromptExecutor:
    ...
    def _process_prompts(
        ...
        # Truncate each prompt to 16384 tokens
        prompts = [truncate_to_max_tokens(p, 16384) for p in prompts]
    ...
    def _run(self, inputs: Dict[str, Any], **kwargs) -> Any:
        ...
        # Log and truncate prompt if necessary
        if isinstance(prompt, list) and len(prompt) == 1 and isinstance(prompt[0], str):
            ...
            print(f"[INFO] Prompt token count before truncation: {orig_tokens}")
            if orig != truncated:
                print(f"[WARNING] Truncated prompt from {orig_tokens} to {truncated_tokens} tokens to fit max tokens.")
            prompt = [truncated]
...
class BasePromptExecutorWithCritique(BasePromptExecutor):
    ...
    def _run(self, inputs: Dict[str, Any], **kwargs) -> List[Any]:
        ...
        # --- Retry mechanism for LLM call ---
        max_retries = 3
        for attempt in range(1, max_retries + 1):
            try:
                raw_output = self._llm.generate(prompt)
                # Consider output invalid if it's None or empty string
                if raw_output is None or (hasattr(raw_output, 'generations') and not raw_output.generations):
                    raise ValueError("LLM returned empty output")
                break  # Success
            except Exception as e:
                print(f"[WARNING] LLM call failed on attempt {attempt}/{max_retries}: {e}")
                if attempt == max_retries:
                    print("[ERROR] LLM call failed after maximum retries. Returning empty result.")
                    return [self._parse_output('', inputs, critique_round=0)]
        if self._stop_at == "llm":
            return raw_output
        ...
        for i in range(self._critique_rounds):
            try:
                ...
                # Retry for critique LLM as well
                for attempt in range(1, max_retries + 1):
                    try:
                        raw_output = self._critique_llm.generate(critique_prompt)
                        if raw_output is None or (hasattr(raw_output, 'generations') and not raw_output.generations):
                            raise ValueError("Critique LLM returned empty output")
                        break
                    except Exception as e:
                        print(f"[WARNING] Critique LLM call failed on attempt {attempt}/{max_retries}: {e}")
                        if attempt == max_retries:
                            print("[ERROR] Critique LLM call failed after maximum retries. Returning empty result.")
                            results.append(self._parse_output('', inputs, critique_round=i + 1))
                            break
                else:
                    continue  # If inner loop didn't break, skip parse
