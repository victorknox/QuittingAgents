...
class StandardVirtualAgentExecutorWithToolkit(AgentExecutorWithToolkit):
    ...
    def _get_simulated_observation(
        ...
        # Track token usage for the emulator
    ...
    def _critique_simulated_observation(
        ...
        for step in range(self.max_allowed_steps):
            ...
            print(f"[DEBUG] Model name detection: model_name={model_name}, type={type(self.llm_critiquer)}")
            print(f"[DEBUG] Model attributes: {[attr for attr in dir(self.llm_critiquer) if not attr.startswith('_')]}")
    ...
    def _take_next_step(
        ...
        for agent_action in actions:
            ...
            if agent_action.tool in name_to_tool_map:
                ...
                print("\n[DEBUG] Simulator LLM called with inputs:")
                for k, v in full_inputs.items():
                    print(f"  {k}: {repr(v)[:500]}")

                # Call the emulator LLM to simulate the execution
                emulator_output = self.llm_simulator_chain.predict(**full_inputs)
                print("[DEBUG] Emulator LLM output:")
                print(repr(emulator_output)[:1000])

                # Emulator cost tracking
                if self.cost_tracker is not None:
                    # Try to get model name from llm_simulator_chain.llm
                    model_name = None
                    llm = getattr(self.llm_simulator_chain, 'llm', None)
                    if llm is not None:
                        ...
                        model_name = 'unknown'
                    from toolemu.utils.misc import get_num_tokens
                    input_tokens = get_num_tokens(str(full_inputs))
                    output_tokens = get_num_tokens(str(emulator_output))
                    print(f"[DEBUG] Emulator cost tracking: model={model_name}, input_tokens={input_tokens}, output_tokens={output_tokens}")
                    self.cost_tracker.add_token_usage('emulator', model_name, input_tokens, output_tokens)
